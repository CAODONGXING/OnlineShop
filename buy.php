...
//如果买过东西又分两种情况：
    ...
    {   //2.此商品第一次购买，就将得到的id和name值组成一个一维数组
        $arr[$pid]=array("pid"=>$pid,"name"=>$name,"num"=>1,"price"=>$price);
    }
}
else
{

    $arr[$pid]=array("pid"=>$pid,"name"=>$name,"num"=>1,"price"=>$price);//重点！！创二维数组原因，方便引用id对应的其他数据
}
$_SESSION["mycar"]=$arr;//购买完后，将此数组重新放入session中，便可以在各个页面看到此session
ob_clean();//清空缓存
header("location:pro_detail.php?page=$pid");//跳转到购物车界面(index.php)
?>indexcar